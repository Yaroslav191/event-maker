...
export default function TabOneScreen() {
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      // getPlaceDetails("ChIJnQx-HSk1-EIR4aaE4hxhGAI", setPlaceInfo);
    } catch (error) {
      console.error("Error getting location:", error);
    }
  };

  const setPlaceName = (coord: any) => {
    try {
      const coordinates = JSON.parse(coord).coordinate;
      // Ensure coord has valid structure before proceeding

      const placeId = "ChIJnQx-HSk1-EIR4aaE4hxhGAI"; // Example placeId

      // Assuming getPlaceDetails is an asynchronous function
      getPlaceDetails(
        placeId,
        setPlaceInfo
  ...
  const handleMapPress = async (coordinate: LatLng) => {
    ...
    getPlaceDetails(coordinate.latitude, coordinate.longitude)
      .then((placeDetails) => {
        newMarker.data = placeDetails.result;
  ...
  const handleMarkerPress = (marker: MarkerInterface) => {
    router.push({
      ...
      params: {
        id: marker.id,
        titleParam: marker.title,
        descriptionParam: marker.description,
        timeParam: marker.time,
      },
    });
  };
          ...
          <MapView
            ...
            {markers.map((marker, index) => (
              <Marker
                ...
                draggable
                coordinate={
                  typeof marker.coordinate == "string"
                    ? JSON.parse(marker.coordinate).coordinate
                    : marker.coordinate
                }
                onDragEnd={(e) =>
              ...
              {markers.map((marker, index) => {
                return (
                  <Pressable
                    ...
                    onPress={
                      () => {}
                      // reverseGeocode(52.27817660470509, 76.95129565955453)
                    }>
                    <View
                      style={{
                        flexDirection: "row",
                      ...
                      <View
                        ...
                        <Text>{marker?.data?.formatted_address}</Text>
                      </View>
                      <View>
                        <Text>1/18</Text>
                      </View>
