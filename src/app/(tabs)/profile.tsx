...
export default function TabTwoScreen() {
   ...
   const [email, setEmail] = useState("Yaroslav");
   const [name, setName] = useState("Роман");
   ...
   const submitData = async (uri: string) => {
      ...
      axios.put(`http://192.168.0.113:8000/updateUser/${userId}`, data);
      ...
      const getUserData = async () => {
         try {
            const response = await axios.get(
               `http://192.168.0.113:8000/user/${userId}`
            );
   ...
   return (
      <View style={styles.container}>
         ...
         >
            <Text style={{ fontWeight: 500 }}>Персональная информация</Text>
